...
{
    /// <summary>
    /// A collection of members returned by the <see cref="TwentyThree.GroupsMembersGetList(string)"/> method.
    /// </summary>
    public sealed class MemberCollection : System.Collections.ObjectModel.Collection<Member>, ITwentyThreeParsable
    {
        /// <summary>
        ...
        void ITwentyThreeParsable.Load(System.Xml.XmlReader reader)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    var m = new Member();
                    ((ITwentyThreeParsable)m).Load(reader);
                    Add(m);
                }
            }
        }
