...
using System.Text;
using System.Xml;
using TwentyThreeNet.Exceptions;
...
namespace TwentyThreeNet
{
    ...
    {
        /// <summary>
        /// Creates a <see cref="TwentyThreeApiException"/> from the response sent back from Flickr.
        /// </summary>
        /// <param name="reader">The <see cref="XmlReader"/> containing the response from Flickr.</param>
        /// <returns>The <see cref="TwentyThreeApiException"/> created from the information returned by Flickr.</returns>
        public static Exception CreateResponseException(XmlReader reader)
        {
            ...
            while (reader.MoveToNextAttribute())
            {
                if (reader.LocalName == "code")
                {
                    ...
                    {
                        throw new TwentyThreeException("Invalid value found in code attribute. Value '" + code + "' is not an integer");
                    }
                    continue;
                }
        ...
        private static TwentyThreeApiException CreateException(int code, string message)
        {
            switch (code)
            {
                case 96:
                ...
                case 115: // XML-RPC error
                    return new TwentyThreeApiException(code, message);
                default:
        ...
        private static TwentyThreeApiException CreateExceptionFromMessage(int code, string message)
        {
            switch (message)
            {
